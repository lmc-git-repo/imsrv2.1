...
export default function Index({auth, accountManagement, queryParams = null, success}) {
    ...
    const sortChanged = (name) => {
        ...
        router.get(route('accountManagement.index'), queryParams, { preserveScroll: true });
    };

    const deleteItem = (accountmanagement) => {
        if (!window.confirm('Are you sure you want to delete this account?')) {
            return;
        }
        router.delete(route('accountManagement.destroy', accountmanagement.id))
    };    
    ...
    <AuthenticatedLayout
        ...
        header={
            <div className='flex justify-between items-center'>
                <h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">List of Accounts</h2>
                ...
                <div className="max-w-8xl mx-auto sm:px-6 lg:px-8">
                    {success && (
                        <div id="alert-border-3" className="flex items-center p-4 mb-4 text-green-800 border-t-4 border-green-300 bg-green-50 dark:text-green-400 dark:bg-slate-800 dark:border-green-800" role="alert">
                            ...
                            <button onClick={() => router.get(route('accountManagement.index'))} type="button" className="ms-auto -mx-1.5 -my-1.5 bg-green-50 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex items-center justify-center h-8 w-8 dark:bg-slate-800 dark:text-green-400 dark:hover:bg-gray-700"  data-dismiss-target="#alert-border-3" aria-label="Close">
                    ...
                    <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            {/* <pre>{JSON.stringify(accountManagement, undefined, 2)}</pre> */}
                                                        ...
                                                        {(auth.user.role === 'super admin' || auth.user.role === 'admin') && (
                                                            <td className="px-3 py-2 text-center text-nowrap">
                                                                <button
                                                                    ...
                                                                    onClick={() => openEditModal(accountmanagement)}
                                                                >
                                                                ...
                                                                <button
                                                                    onClick={(e) => deleteItem(accountmanagement)}
                            ...
                            <Pagination 
                                links={accountManagement.meta.links}
                                queryParams={{
                                    search: searchQuery,                                    
                                }}
                            />
