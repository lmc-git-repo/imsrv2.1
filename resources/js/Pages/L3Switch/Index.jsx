...
export default function Index({
    ...
    // Modal functions
    const openModal = (l3switch, e) => {
        e.preventDefault();
        setSelected(l3switch);
        setShowModal(true);
    };
    ...
    const openEditModal = (l3switch) => {
        setSelectedEdit(l3switch);
    ...
    const sortChanged = (name) => {
        ...
        router.get(route('l3sw.index'), queryParams, { preserveScroll: true });
    };

    const deleteL3Switch = (l3switch) => {
        if (!window.confirm('Are you sure you want to delete this L3 Switch?')) {
            return;
        }
        router.visit(route('l3sw.destroy', l3switch.id), { method: 'delete' });
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className='flex justify-between items-center'>
                    <h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">List of L3 Switches</h2>
                ...
                <div className="max-w-8xl mx-auto sm:px-6 lg:px-8">
                    {success && (
                        <div id="alert-border-3" className="flex items-center p-4 mb-4 text-green-800 border-t-4 border-green-300 bg-green-50 dark:text-green-400 dark:bg-slate-800 dark:border-green-800" role="alert">
                            ...
                            <button onClick={() => router.get(route('l3sw.index'))} type="button" className="ms-auto -mx-1.5 -my-1.5 bg-green-50 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex items-center justify-center h-8 w-8 dark:bg-slate-800 dark:text-green-400 dark:hover:bg-gray-700" data-dismiss-target="#alert-border-3" aria-label="Close">
                        ...
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            <div className="overflow-auto">
                                <div className="flex justify-between items-center py-2">
                                    <div>
                                        <label htmlFor="l3switch-search" className="sr-only">
                                            Search L3 Switches
                                        </label>
                                        <TextInput
                                            id="l3switch-search"
                                            name="l3switch-search"
                                            className="w-full"
                                ...
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        {l3switches.data && l3switches.data.length > 0 ? (
                                            l3switches.data.map(l3switch => (
                                                <tr className="bg-white border-b dark:bg-slate-800 dark:border-gray-700" key={l3switch.id}>
                                                    <th className="px-3 py-2 text-nowrap">
                                                        <button
                                                            className="text-left hover:text-white hover:underline cursor-pointer"
                                                            onClick={(e) => openModal(l3switch, e)}
                                                            aria-label={`View ${l3switch.device_name} details`}
                                                        >
                                                    ...
                                                    <td className="px-3 py-2 text-center">
                                                        {(auth.user.role === 'super admin' || auth.user.role === 'admin') && (
                                                            <button
                                                                ...
                                                                aria-label={`Edit ${l3switch.device_name}`}
                                                            >
                                                        ...
                                                        {(auth.user.role === 'super admin' || auth.user.role === 'admin') && (
                                                            <button
                                                                ...
                                                                aria-label={`Delete ${l3switch.device_name}`}
                                                            >
                                                                <span className='flex items-center justify-center'>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
