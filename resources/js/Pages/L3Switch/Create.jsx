...
const CreateL3Switch = forwardRef(function CreateL3Switch({ show, onClose }, ref) {
  ...
  const onSubmit = (e) => {
    ...
    post(route('l3sw.store'), {
      ...
      onError: () => {
        console.error('[❌ ERROR] Failed to create L3 switch.');
      },
      onFinish: () => {
        setLoading(false);
        setSubmitted(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div
        className="bg-slate-700 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto"
        ref={ref}
      >
        {/* Header */}
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-slate-600">
          <h2 className="text-lg sm:text-xl font-semibold text-white">Add New L3 Switch</h2>
          <button
            onClick={onClose}
        ...
        {/* Body */}
        <div className="p-6">
          <form onSubmit={onSubmit} className="space-y-4" name="create-l3switch-form">
            {/* Device Name */}
            <div>
...
CreateL3Switch.displayName = 'CreateL3Switch';
export default CreateL3Switch;