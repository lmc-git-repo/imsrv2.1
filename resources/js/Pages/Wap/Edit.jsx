...
const EditWAP = forwardRef(function EditWAP({ show, onClose, selectedWAP }, ref) {
  ...
  useEffect(() => {
    if (selectedWAP) {
      const newData = {
        device_name: selectedWAP.device_name || '',
        model: selectedWAP.model || '',
        ip_address: selectedWAP.ip_address || '',
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (!selectedWAP?.id || loading || submitted) return;

    setLoading(true);
    setSubmitted(true);
    put(route('wap.update', selectedWAP.id), {
      onSuccess: () => {
        setLoading(false);
        setSubmitted(false);
        onClose();
  ...
  if (!show || !selectedWAP) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-slate-700 rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-600">
          <h2 className="text-xl font-semibold text-white">Edit WAP - {selectedWAP.device_name || 'Unknown'}</h2>
          <button
            onClick={onClose}
