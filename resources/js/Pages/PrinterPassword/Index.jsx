...
export default function Index({auth, printerPassword, queryParams = null, success}) {
    ...
    const sortChanged = (name) => {
        ...
        router.get(route('printerPassword.index'), queryParams, { preserveScroll: true });
    };

    const deleteItem = (printerpassword) => {
        if (!window.confirm('Are you sure you want to delete this printer account?')) {
            return;
        }
        router.delete(route('printerPassword.destroy', printerpassword.id))
    };    
    ...
    <AuthenticatedLayout
        ...
        header={
            <div className='flex justify-between items-center'>
                <h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Printer Passwords</h2>
                ...
                <div className="max-w-8xl mx-auto sm:px-6 lg:px-8">
                    {success && (
                        <div id="alert-border-3" className="flex items-center p-4 mb-4 text-green-800 border-t-4 border-green-300 bg-green-50 dark:text-green-400 dark:bg-slate-800 dark:border-green-800" role="alert">
                            ...
                            <button onClick={() => router.get(route('printerPassword.index'))} type="button" className="ms-auto -mx-1.5 -my-1.5 bg-green-50 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex items-center justify-center h-8 w-8 dark:bg-slate-800 dark:text-green-400 dark:hover:bg-gray-700"  data-dismiss-target="#alert-border-3" aria-label="Close">
                    ...
                    <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            {/* <pre>{JSON.stringify(printerPassword, undefined, 2)}</pre> */}
                            <div className="overflow-auto">
                                ...
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        ...
                                        ) : printerPassword.data && printerPassword.data.length > 0 ? (
                                                printerPassword.data.map(printerpassword => (
                                                    <tr className="bg-white border-b dark:bg-slate-800 dark:border-gray-700" key={printerpassword.id}>
                                                        {/* <td className="px-3 py-2">{printerpassword.id}</td> */}
                                                        <th className="px-3 py-2 hover:underline hover:text-white">
                                                            <Link href="#" onClick={(e) => openModal(printerpassword, e)}>
                                                                {printerpassword.dept}
                                                            </Link>
                                                        </th>
                                                        <td className="px-3 py-2">{printerpassword.password}</td>
                            ...
                            <Pagination 
                                links={printerPassword.meta.links}
                                queryParams={{
                                    search: searchQuery,                                    
                                }}
                            />
